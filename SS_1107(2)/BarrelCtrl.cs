...
public class BarrelCtrl : MonoBehaviour
{
    ...
    void OnDamage(object[] _params) //드럼통에 물리력을 주기 위해서 입사각을 구함
    {
        Vector3 hitPos = (Vector3)_params[0];
        Vector3 firePos = (Vector3)_params[3];
        Vector3 incomeVector = (hitPos - firePos).normalized; //입사각 //normalized 정규화(1)
        rb.AddForceAtPosition(incomeVector * 1000.0f, hitPos); //AddForceAtPosition = hit된 위치에 힘을 가한다
        if (++hitCount == 3)
        {
            ExpBarrel();
        }
